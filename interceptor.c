/*
 * ============================================================================================
 * File Name : interceptor.c
 * Name	: Seungkyu Kim
 * Created Date	: Sep.25.2015
 * Modified Date : Sep.28.2015
 * Description	: implement a new system call named my_syscall, 
 *   			  which will allow you to send commands from userspace, 
 *  			  to intercept another pre-existing system call (like read, write, open, etc.). 
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	if (check_pid_monitored(reg.ax, current->pid) || table[reg.ax].monitored == 2) {
 ...
 * 
 * Implement this function, to handle all 4 commands correctly.
 *
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	// to get current pid of the calling function use current->pid
	...
	// check for validation of pid	
	if (find_vpid(pid), PIDTYPE_PID || pid < 0) {
		return -EINVAL;
	}

	// Check that the caller has the right permissions
	if (current_uid() != 0){
	...
	if (table[syscall].intercepted == 0){
		...
		if(cmd == REQUEST_SYSCALL_RELEASE) {
			return -EINVAL;
		// cannot stop monitoring for a pid that isnot being monitored or syscall has not been intercepted yet	
		} else if (cmd == REQUEST_STOP_MONITORING && check_pid_monitored(syscall, pid) == 0){
			return -EINVAL;
		}
	// check intercepting a system call that is already intercepted
	} else if ((table[syscall].intercepted == 1) && (cmd == REQUEST_SYSCALL_INTERCEPT)) {
		return -EBUSY;
	// check monitoring a pid that is already being monitored
	} else if ((cmd == REQUEST_START_MONITORING) && check_pid_monitored(syscall, pid) == 1){
		return -EBUSY;
	}

	// Done error checking up to this part
...
static int init_function(void) {
	...
	for(i = 0; i < NR_syscalls; i++) {
		//Save Pointer to  function at syscal index
	...
	// Hijack custom syscall and exit!
	spin_lock(&calltable_lock);
	set_addr_rw((unsigned long)sys_call_table);
	
	sys_call_table[MY_CUSTOM_SYSCALL] = my_syscall;
	sys_call_table[__NR_exit_group] = sys_call_table[__NR_exit_group];
